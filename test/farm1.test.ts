...
const RANCEPerBlock = 1000000000;
...
describe("Farming Contract Tests", () => {
  ...
  beforeEach( async () => {
    ...
    Farm = await ethers.getContractFactory("MasterRANCE");
    //farm = await Farm.deploy(RANCE.address, RANCEPerBlock, 0)
    farm = await upgrades.deployProxy(Farm, [RANCE.address, RANCEPerBlock, 0, wallet.address], {kind: "uups"})
  ...
  it("should initialise contract state variables", async () => {
    expect(await farm.RANCE()).to.equal(RANCE.address)
    expect(await farm.RANCEPerBlock()).to.equal(RANCEPerBlock)
  ...
  it("should add the RANCE Token Pool", async () => {
    ...
    expect(pool.accRANCEPerShare.toString()).to.equal("0")
  })
  ...
  context("when user deposits when wallet is empty", async () => {
    ...
    it("should update user info", async () => {
      ...
      expect(await farm.pendingRANCE(0, await PrederA.getAddress())).to.equal(0)
    })
    ...
    it("should update pool", async () => {
      ...
      expect(poolAfter.accRANCEPerShare).to.equal(poolBefore.accRANCEPerShare)
    })
    ...
    it("should update user pending RANCE when wallet increases balance", async () => {
      ...
      const pool = await farm.poolInfo(0)
      const pending = (multiplier*RANCEPerBlock).toString();
      expect(await farm.pendingRANCE(0, await PrederA.getAddress()))
        .to.equal(
          user.amount.mul(pool.accRANCEPerShare).div((BigNumber.from(10).pow(30))).sub(user.rewardDebt)
        )
    })
    ...
    it("should mass update pools", async () => {
      ...
      expect(newPool.accRANCEPerShare, "AccRANCEShare not correct").to.equal(
        oldPool.accRANCEPerShare.add(
          _multiplier
          .mul(RANCEPerBlock)
    ...
    it("it should withdraw user rewards with withdraw function", async () => {
      await RANCE.transfer(wallet.address, (10**17).toString());
      const pending: BigNumber = await farm.pendingRANCE(0, await PrederA.getAddress());
  ...
  context("when user deposits when wallet is not empty", async () => {
    ...
    it("should update user info", async () => {
      const userInfo = await farm.userInfo(0, await PrederA.getAddress())
      const pending = await farm.pendingRANCE(0, await PrederA.getAddress());
      expect(userInfo.amount).to.equal(depositA)
      expect(userInfo.rewardDebt).to.equal(0)
      expect(await farm.pendingRANCE(0, await PrederA.getAddress())).to.equal(pending);
    })
    ...
    it("should update pool", async () => {
      ...
      expect(poolAfter.lastRewardBlock).to.gt(poolBefore.lastRewardBlock)
      expect(poolAfter.accRANCEPerShare).to.equal(
        poolBefore.accRANCEPerShare.add(
  ...
  context("Compound rewards", () => {
    ...
    it("should compound rewards", async () => {
      let pending: BigNumber = await farm.pendingRANCE(0, await PrederA.getAddress());
